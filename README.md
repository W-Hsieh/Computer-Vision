...
- **C. Siamese Networks**
  - Following the feature extraction process, we proceeded to train our model using the Siamese Networks. Siamese Networks comprise multiple sister networks that share weights. Each of these networks generates embedding vectors for their individual inputs. In the realm of supervised similarity learning, these networks are fine-tuned to amplify the contrast (or distance) between the embeddings of inputs from distinct classes. Simultaneously, they aim to reduce the distance between embeddings of inputs from the same class. This approach crafts an embedding space that mirrors the class delineation of the training inputs. For distance computation, we have chosen to apply both Manhattan Distance (L1) and Euclidean Distance (L2) separately. Additionally, to combat overfitting, we employed dropout as a regularization technique. Dropout operates by randomly deactivating neurons within the neural network during each training iteration. By deactivating varying sets of neurons, it effectively mimics the training of multiple diverse neural networks.
